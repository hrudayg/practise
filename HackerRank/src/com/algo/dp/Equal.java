...
public class Equal {
	 ...
	 * persons value.
	 * 
	 * For example - Out of 4 people in a competition,3 gets selected for next
	 * round --> 1 person loses. Out of 4 people in a competition, 1 person
	 * loses --> 3 gets selected for next round
	 * 
	 * Actually we should reduce each of the array value to the min or min - 1
	 * or min - 2 or min - 4 --- till each of the value is greater than 0 min -
	 * 5 will again increase the count because it actually adds n to the result
	 * of min where n is the length of the cookie array. so we will consider
	 * only till min - 4
	 * 
	 ...
	 * 
	 * Now instead of considering all the iterations from min to min - 4 we can
	 * have a base values as 0 , 1, 2 .
	 * 
	 * first we sort the array. We then start with first value in the cookie
	 * array and bring all other vaues to the first value .
	 * 
	 */
